...
// ─────────────────────────────────────────────────────────────────────────────
// POL-06 Sync endpoint JSON limit must be >= 10mb (photos are base64)
// Checks backend/src/server.js for express.json({ limit }) on sync push.
const serverPath06 = path.join(root, "backend", "src", "server.js");
if (fs.existsSync(serverPath06)) {
  const serverSrc = fs.readFileSync(serverPath06, "utf8");
  // Look for the sync push route limit: e.g. express.json({ limit: "50mb" })
  // We need a limit on /api/sync/pets/push that is >= 10mb
  const syncLimitMatch = serverSrc.match(/sync\/pets\/push.*?express\.json\(\s*\{[^}]*limit\s*:\s*["'](\d+)(mb|kb|gb)["']/is)
    || serverSrc.match(/express\.json\(\s*\{[^}]*limit\s*:\s*["'](\d+)(mb|kb|gb)["']\s*\}\s*\)[\s\S]{0,200}sync\/pets\/push/is);

  if (!syncLimitMatch) {
    // Check if there's a line with both sync/pets/push and a limit
    const lines06 = serverSrc.split(/\r?\n/);
    let foundSyncLimit = false;
    for (let i = 0; i < lines06.length; i++) {
      const line = lines06[i];
      if (line.includes("sync/pets/push") && /limit/.test(line)) {
        const m = line.match(/["'](\d+)(mb|kb|gb)["']/i);
        if (m) {
          const val = parseInt(m[1], 10);
          const unit = m[2].toLowerCase();
          const mb = unit === "gb" ? val * 1024 : unit === "kb" ? val / 1024 : val;
          if (mb < 10) {
            console.error(`❌ POL-06: Sync push JSON limit is ${val}${unit} (<10mb). Photos need >= 10mb.`);
            process.exit(1);
          }
          foundSyncLimit = true;
        }
    ...
    if (!foundSyncLimit) {
      console.error("❌ POL-06: No explicit JSON limit found for /api/sync/pets/push. Photos need >= 10mb.");
      process.exit(1);
    }
  } else {
    const val = parseInt(syncLimitMatch[1], 10);
    const unit = syncLimitMatch[2].toLowerCase();
    const mb = unit === "gb" ? val * 1024 : unit === "kb" ? val / 1024 : val;
    if (mb < 10) {
      console.error(`❌ POL-06: Sync push JSON limit is ${val}${unit} (<10mb). Photos need >= 10mb.`);
      process.exit(1);
